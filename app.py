            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=self.request_timeout) as session:
                    ...
                    async with session.post(
                        ...
                        # FIXED: Check for successful payment method addition
                        if confirm_res.status == 200:
                            # Check for various success indicators
                            success_indicators = [
                                confirm_json.get("success", False),
                                "Payment method successfully added" in confirm_response_text,
                                "Payment method added" in confirm_response_text,
                                confirm_json.get("data", {}).get("status") == "succeeded",
                                confirm_json.get("data", {}).get("status") == "requires_action"
                            ]
                            ...
                            # Site returned non-200
                            error_message = f"HTTP {confirm_res.status}: Card declined"
                            logger.error(f"Site HTTP error: {confirm_res.status}")
                            return False, status, error_message
