...
"""
    ...
    async def chk_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            result, error_message = await self.process_line(user_id, combo, asyncio.Semaphore(1), update, is_single_check=True)
            if result:
                # For single check, send approval message
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=self.request_timeout) as session:
                    ...
                    async with session.post(
                        ...
                        if stripe_res.status != 200 or 'id' not in stripe_json:
                            ...
                            if 'error' in stripe_json:
                                error_message = f"Stripe: {stripe_json['error'].get('message', 'Unknown error')}"
                            else:
                                error_message = "Stripe: Payment method creation failed"
                    ...
                    async with session.post(
                        ...
                        if confirm_res.status == 200 and confirm_json.get("success", False) and confirm_json.get("data", {}).get("status", "") == "succeeded":
                            check_time = (datetime.now() - start_time).total_seconds()
                            return combo, None
                        else:
                            # Capture actual site error response
                            if 'data' in confirm_json and 'message' in confirm_json['data']:
                                error_message = f"Site: {confirm_json['data']['message']}"
                            elif 'error' in confirm_json:
                                error_message = f"Site: {confirm_json['error']}"
                            else:
                                error_message = "Site: Card declined - Unknown reason"
