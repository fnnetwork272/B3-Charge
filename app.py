...
"""
    ...
    async def chk_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            result, error_message = await self.process_line(user_id, combo, asyncio.Semaphore(1), update)
            if result:
                await update.message.reply_text(f"✅ APPROVED\n{combo}")
            else:
                # Send detailed declined message
                bin_info = await self.fetch_bin_info(combo[:6])
                await self.send_declined_message(update, combo, bin_info, error_message)
        except Exception as e:
        ...
        try:
            with open(filename, 'r') as f:
                ...
                for future in asyncio.as_completed(tasks):
                    result, error_message = await future
                    ...
                    if result:
                        ...
                        # Send declined message for each declined card
                        bin_info = await self.fetch_bin_info(result[:6]) if result else None
                        await self.send_declined_message(update, result, bin_info, error_message)
...
"""
                ...
                async with aiohttp.ClientSession(timeout=self.request_timeout) as session:
                    ...
                    async with session.post(
                        ...
                        if 'id' not in stripe_json:
                            error_message = stripe_json.get('error', {}).get('message', 'Unknown Stripe error')
                    ...
                    async with session.post(
                        ...
                        if confirm_json.get("success", False) and confirm_json.get("data", {}).get("status", "") == "succeeded":
                            ...
                            error_message = confirm_json.get("data", {}).get("message", "Card declined")
                            return False, error_message
    ...
    async def send_declined_message(self, update, combo, bin_info, error_message):
        check_time = random.uniform(3.0, 10.0)  # Simulate check time for declined cards
        user = update.effective_user
